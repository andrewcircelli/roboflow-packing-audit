...
frame center x=1080). cx is the center of the detected parcel:

    frame_00104   cx= 734   54% of frame   entering, off-center
    ...
    frame_00261   cx=1343   71%            leaving, off-center
...
# Half-width of the read window either side of frame center, in pixels.
...
def build_zone(frame_width: int, frame_height: int) -> np.ndarray:
    ...
    left = center_x - ZONE_HALF_WIDTH_PX
    right = center_x + ZONE_HALF_WIDTH_PX
    return np.array(
...
def is_in_zone(
    ...
    because the zone was calibrated against the parcel's center point. A
    ...
    the band. The band is a target for its center, not a container for it.
    """
